...
REG_FNS_AND_TEMPS = {
  ...
  "exp1": (lambda x, a, b: a*np.exp(b*x), TP("$a*e**($b*@x)"), TP("ln(@y/$a)/$b")),
  "exp2": (lambda x, a, b: a*b**x, TP("$a*$b**@x"), TP("ln(@y/$a)/ln($b)")),
  "pow" : (lambda x, a, b: a*x**b, TP("$a*@x**$b"), TP("(@y/$a)**(1/$b)")),
...
TOKENS_ARG_POSITIONING = {
  ...
  '@fact': 'lf', '@frac': 'bi', '@grad': 'lf', '@ln': 'bi-const',
